...
var require_coerce = __commonJS({
  "node_modules/.pnpm/semver@7.6.2/node_modules/semver/functions/coerce.js"(exports2, module2) {
    ...
    var coerce = (version, options) => {
      ...
      return parse(`${major2}.${minor}.${patch}${prerelease3}${build}`, options);
    };
...
async function metadata(ctx, crate, force = false) {
  ...
  if (!m || m.createdAt > (/* @__PURE__ */ new Date()).getUTCMilliseconds() - 1e3 * 60 * 30) {
    const m2 = await sparseIndexMetadata(crate);
    ctx.globalState.update(key, m2);
    return m2;
  }
...
async function dependencyDecoration(ctx, input) {
  ...
  if (!res.current) {
    res.status = "Not Installed" /* NOT_INSTALLED */;
    return { ctx, decoration: res };
  }
  if ((0, import_semver3.prerelease)(res.current) === null) {
    if (input.currentVersion !== m.latestStable) {
      res.status = "Outdated" /* OUTDATED */;
      res.latest = m.latestStable;
      if ((0, import_semver3.major)(res.current) === (0, import_semver3.major)(m.latestStable)) {
        return { ctx, decoration: res };
      } else {
        for (let v of m.versions) {
          if ((0, import_semver3.satisfies)(v, res.current)) {
            res.currentMax = v;
            return { ctx, decoration: res };
          }
        }
  ...
  if (input.currentVersion !== m.latestPrerelease) {
    ...
    if ((0, import_semver3.satisfies)(m.latestPrerelease, res.current)) {
      return { ctx, decoration: res };
    } else {
      for (let v of m.versions) {
        if ((0, import_semver3.satisfies)(v, res.current)) {
...
async function dependenciesDiagnostics(ctx, input) {
  ...
  for await (let d of input) {
    ...
    for (let v of m.versions) {
      if ((0, import_semver4.satisfies)(v, d.inputVersion)) {
        satisfiedVersion = v;
        break;
      }
    }
...
var DependenciesTraverser = class extends DependenciesWalker {
  ...
  identifiers = [];
  doc;
  constructor(tree, doc, nodeStore, version) {
    super(tree, version);
    this.doc = doc;
  ...
  onCrate(id, node, table, platform) {
    ...
    this.nodeStore.set(input.id, { value: this.doc.getText(node.range), table, range: node.range, denpendencyType: "Crate" /* CRATE */ }, this.getVersion());
    this.nodeStore.setNode(input.id, { value: this.doc.getText(node.range), table, range: node.range, denpendencyType: "Crate" /* CRATE */ });
    ...
    for (let child of node.children) {
      if (child.name === "version") {
        ...
        input.inputVersion = version;
        this.nodeStore.setNode(nodeId(input.id, child.name), { value: version, range: child.range, table, denpendencyType: "Version" /* VERSION */ });
        continue;
      }
      if (child.name === "features") {
        if (child.children.length !== 0) {
          for (let grandChild of child.children) {
            ...
            this.nodeStore.setNode(nodeId(input.id, child.name, grandChild.name), { value: f, range: grandChild.range, table, denpendencyType: "Feature" /* FEATURE */ });
            input.features.push(f);
          }
        } else {
          const f = this.doc.getText(squezze(child.range));
          this.nodeStore.setNode(nodeId(input.id, child.name), { value: f, range: child.range, table, denpendencyType: "Feature" /* FEATURE */ });
          input.features.push(f);
        }
        continue;
      }
...
var NodeStore = class {
  ...
  addUpdated(id, version) {
    ...
    if (v === void 0 || v < version) {
      this.updated.set(id, version);
    }
  }
  checkAndDelDirty(id, version) {
    if (this.added.has(id)) {
      this.added.delete(id);
      return true;
    }
    const v = this.updated.get(id);
    if (v === void 0 || version < v) {
      return false;
    }
...
function outdatedDecoration(deco) {
  if (deco.currentMax !== "" && deco.current === deco.currentMax) {
    return import_vscode8.window.createTextEditorDecorationType({
      after: {
        contentText: "\u{1F512} " + deco.current + ", " + deco.latest,
        color: "orange",
        margin: "0 0 0 4em"
        // Add some margin to the left
      }
...
function notInstalledDecoration() {
  return import_vscode8.window.createTextEditorDecorationType({
    after: {
...
async function cargoTree(path) {
  ...
  return parseCargoTreeOutput(tree.unwrap());
}
...
var DiagnosticStore = class {
  ...
  state = {};
  constructor() {
    this.collection = import_vscode9.languages.createDiagnosticCollection("crates-cmp");
  }
  addRaw(path, range, d) {
    if (this.state[path]) {
      this.state[path].push(new import_vscode9.Diagnostic(range, d.message, d.severity));
      return;
    }
    this.state[path] = [new import_vscode9.Diagnostic(range, d.message, d.severity)];
  }
  add(path, ...d) {
    if (this.state[path]) {
  ...
  clearSeverity(uri, severity) {
    if (!this.state[uri.path]) return void 0;
...
var Listener = class {
  ...
  decorationStore = new DecorationStore();
  //track toml nodes
  ...
  async onChange(document) {
    ...
    if (currentDepsResult.isErr()) {
      ...
      let clear = [];
      diagnostics.filter((d) => this.nodeStore.checkAndDelDirty(d.diagnostic.id, d.ctx.version)).forEach((d) => {
    ...
    this.diagnosticStore.clearSeverity(ctx.uri, import_vscode10.DiagnosticSeverity.Error);
    if (this.nodeStore.isClean()) {
      return;
    }
      ...
      if (currentDepts[dep.tableName]) {
        ...
        if (deps[crateName]) {
          ...
          if (found[crateName]) {
            ...
            found[crateName] = { versions: /* @__PURE__ */ new Set([dep.currentVersion]), ids: [dep.id] };
          }
          continue;
        }
      }
  ...
  setWaitingNodes(ids) {
    for (let id of ids) {
      this.decorate(id, { id, current: "", currentMax: "", latest: "", status: "Loading" /* LOADING */ });
    }
  }
  clearDecoration(ids) {
  ...
  decorate(id, deco) {
    ...
    if (d) {
      if (d.latest === deco.latest && d.status === deco.status) {
        if (this.nodeStore.range(id)) {
          import_vscode10.window.activeTextEditor?.setDecorations(d.decoration, [this.nodeStore.range(id)]);
        }
        return d.decoration;
      } else {
    ...
    if (this.nodeStore.range(id)) {
      import_vscode10.window.activeTextEditor?.setDecorations(dt, [this.nodeStore.range(id)]);
    }
  }
  async onDidChangeTextDocument(document) {
...
var RustAnalyzer = class {
  ...
  dependencies = {};
  async init() {
    const extension = import_vscode11.extensions.getExtension("rust-lang.rust-analyzer");
    if (!extension) {
...
async function activate(context) {
  ...
  let task = new import_vscode12.Task(def, import_vscode12.TaskScope.Workspace, "cargo update", "crates-cmp", new import_vscode12.ShellExecution("cargo update"));
  let taskc = import_vscode12.tasks.registerTaskProvider("cargo", {
    provideTasks: () => {
      return [task];
    },
    resolveTask: (task2, token) => {
      return task2;
    }
  });
  ...
  context.subscriptions.push(
    ...
    import_vscode12.window.onDidChangeActiveTextEditor(listener.onDidChangeActiveEditor, listener),
