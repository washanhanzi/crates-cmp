...
// src/entity/toml.ts
function dependencyItemKey(crateName, type, item) {
  switch (type) {
    case "Crate" /* CRATE */:
...
function crateItemKey(crateName) {
  return dependencyItemKey(crateName, "Crate" /* CRATE */);
}
function versionItemKey(crateName, version) {
  return dependencyItemKey(crateName, "Version" /* VERSION */, version);
}
function featureItemKey(crateName, feature) {
  return dependencyItemKey(crateName, "Feature" /* FEATURE */, feature);
}
...
function newLatestDecoration(crateName, latest) {
  return {
...
function newOutdatedDecoration(crateName, latest) {
  return {
    key: crateItemKey(crateName),
...
var CargoTomlWalker = class {
  ...
  onDependencies(node, table, platform) {
  }
  onFeatures(node) {
  }
  onWorkspace(node) {
      ...
      switch (node.name) {
        ...
        case "dependencies":
          if (this.enterDependencies(node, "dependencies" /* DEPENDENCIES */)) {
            this.onDependencies(node, "dependencies" /* DEPENDENCIES */);
          }
          break;
        case "dev-dependencies":
          if (this.enterDependencies(node, "dev-dependencies" /* DEV_DEPENDENCIES */)) {
            this.onDependencies(node, "dev-dependencies" /* DEV_DEPENDENCIES */);
          }
          break;
        case "build-dependencies":
          if (this.enterDependencies(node, "build-dependencies" /* BUILD_DEPENDENCIES */)) {
            this.onDependencies(node, "build-dependencies" /* BUILD_DEPENDENCIES */);
          }
          break;
        case "target":
          if (this.enterDependencies(node, "target.dependencies" /* TARGET_DEPENDENCIES */)) {
            for (let child of node.children[0].children) {
              this.onDependencies(child, "target.dependencies" /* TARGET_DEPENDENCIES */, node.children[0].name);
            }
          }
        case "features":
  ...
  decoration(name, deco) {
    ...
    switch (deco.status) {
      case "Latest" /* LATEST */:
        ...
        this.decorationState[name] = { key, status: deco.status, decoration: newLatest };
        return newLatest;
