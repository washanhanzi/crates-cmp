...
function execCargoAuditAsync(command) {
  return new Promise((resolve, reject) => {
    const child = (0, import_child_process.exec)(command);
    let stdoutBuffer = "";
    let stderrBuffer = "";
...
var ENABLE_CARGO_AUDIT = "cargo.audit.enable";
var Config = class {
  ...
  updateCargoAudit() {
    const enableAudit = import_vscode.workspace.getConfiguration("crates-cmp").get(ENABLE_CARGO_AUDIT);
...
var DependenciesTraverser = class extends DependenciesWalker {
  ...
  onCrate(id, node, table, platform) {
    ...
    for (let child of node.children) {
      ...
      if (child.name === "path") {
        const id2 = nodeId(input.id, child.name);
        const path2 = this.doc.getText(squezze(child.range));
        input.path = { id: id2, dependencyId: input.id, value: path2 };
        this.docTree.visitNode(id2, { id: id2, table, value: path2, range: child.range, dependency: { dependencyId: input.id, key: "path" /* PATH */ } });
        rangeIds.push(id2);
      }
    }
    this.docTree.addDependency(input);
...
var DiagnosticStore = class {
  ...
  delete(uri, id) {
    ...
    if (m) {
      ...
      if (success) {
        const ds = Array.from(m.values()).map((m2) => m2.diagnostic);
...
async function getPathForExecutable(executableName) {
...
async function findExecutablePath(executableName) {
...
async function isExecutableInPath(exec2) {
  const paths = process.env["PATH"]?.split(path.delimiter) || [];
  const candidates = paths.flatMap((dirInPath) => {
    const candidate = path.join(dirInPath, exec2);
...
async function isFileAtPath(path2) {
  return isFileAtUri(vscode.Uri.file(path2));
}
async function isFileAtUri(uri) {
  try {
...
async function cargoTree(path2) {
  const executable = await cargoPath();
  const tree = await execAsync(`${executable} tree --manifest-path ${path2} --depth 1 --all-features`).catch((e) => {
    throw e;
  });
  return parseCargoTreeOutput(tree);
}
async function cargoAudit(path2) {
  const executable = await cargoPath();
  const auditOutput = await execCargoAuditAsync(`${executable} audit --json -f ${path2}`).catch((e) => {
    throw e;
  });
  ...
  async afterCargoErr(ctx) {
    ...
    diagnostics.filter((d) => {
      ...
      this.diagnostic.add(
        ...
        {
          id: depId,
          state: new MyDiag("Dependency" /* DEPENDENCY */),
          diagnostic: new import_vscode11.Diagnostic(this.docTree.range(rangeId), message, severity)
        }
      );
    });
      ...
      for (let d of duplicated) {
        this.diagnostic.add(
          ctx.path,
          {
            id: d.dependencyId,
            state: new MyDiag("Dependency" /* DEPENDENCY */),
            diagnostic: new import_vscode11.Diagnostic(this.docTree.range(d.rangeId), `Found multiple versions: ${d.crates.join(", ")}`, import_vscode11.DiagnosticSeverity.Information)
          }
        );
      }
      this.diagnostic.render(ctx.uri);
    } else {
      this.diagnostic.clear(ctx.uri, new MyDiag("Dependency" /* DEPENDENCY */));
    }
...
var Audit = class {
  ...
  async initialize() {
    ...
    const lockFileList = await import_vscode12.workspace.findFiles("**/Cargo.lock");
  ...
  async onLockFileChange(e) {
    if (!config.getEnableAudit()) return;
    const res = await cargoAudit(e.path);
    this.state.set(e.path, res);
    this.updateDiagnostic();
  }
  async onLockFileCreate(e) {
    if (!config.getEnableAudit()) return;
    const res = await cargoAudit(e.path);
    this.state.set(e.path, res);
    this.updateDiagnostic();
  }
  async onLockFileDelete(e) {
    if (!config.getEnableAudit()) return;
    this.state.delete(e.path);
    this.updateDiagnostic();
  }
  updateDiagnostic() {
    if (!import_vscode12.window.activeTextEditor) return true;
    if (!import_vscode12.window.activeTextEditor.document.fileName.endsWith("Cargo.toml")) true;
    const path2 = import_vscode12.window.activeTextEditor.document.uri.path;
    let severity = import_vscode12.DiagnosticSeverity.Hint;
    let message = "";
...
`;
        severity = import_vscode12.DiagnosticSeverity.Error;
...
`;
      ...
      if (v?.warnings?.unsound && v.warnings.unsound.length > 0) {
        if (severity !== import_vscode12.DiagnosticSeverity.Error) {
          severity = import_vscode12.DiagnosticSeverity.Warning;
        }
...
`;
    ...
    if (severity === import_vscode12.DiagnosticSeverity.Hint) return;
    this.diagnostic.add(path2, {
      id: "package",
      state: new MyDiag("Audit" /* AUDIT */),
      diagnostic: {
        message,
        range: new import_vscode12.Range(0, 1, 0, 10),
        severity
      }
    });
    this.diagnostic.render(import_vscode12.window.activeTextEditor.document.uri);
  }
  hoverText() {
  }
};
var audit = new Audit(diagnosticStore);
...
var Listener = class {
  ...
  async onDidLockFileChange(e) {
    audit.onLockFileChange(e);
    if (this.state.noResumeDepTree()) return;
    await this.state.parseDependencies(this.ctx, import_vscode13.window.activeTextEditor.document);
  }
...
var RustAnalyzer = class {
  ...
  dependencies = {};
  async init() {
    const extension = import_vscode14.extensions.getExtension("rust-lang.rust-analyzer");
    if (!extension) {
...
var CargoTomlAction = class {
  ...
  static providedCodeActionKinds = [
    import_vscode15.CodeActionKind.RefactorRewrite
  ];
        ...
        case "version" /* VERSION */:
          ...
          switch (state) {
            case "NotExist" /* NOT_EXIST */:
              const notExistAction = new import_vscode15.CodeAction(dep.version.latest, import_vscode15.CodeActionKind.QuickFix);
              notExistAction.edit = new import_vscode15.WorkspaceEdit();
              notExistAction.edit.replace(document.uri, node.range, '"' + dep.version.latest + '"');
              actions.push(notExistAction);
              break;
            case "Outdated" /* OUTDATED */:
              const outdatedAction = new import_vscode15.CodeAction(dep.version.latest, import_vscode15.CodeActionKind.RefactorRewrite);
              outdatedAction.edit = new import_vscode15.WorkspaceEdit();
...
async function activate(context) {
  ...
  import_vscode16.workspace.onDidChangeConfiguration(config.onChange, config);
  const listener = new Listener(context);
  audit.initialize();
  const watcher = import_vscode16.workspace.createFileSystemWatcher("**/Cargo.lock");
  context.subscriptions.push(
    import_vscode16.window.onDidChangeActiveTextEditor(listener.onDidChangeActiveEditor, listener),
    import_vscode16.workspace.onDidSaveTextDocument(listener.onDidSaveTextDocument, listener),
    import_vscode16.workspace.onDidCloseTextDocument(listener.onDidCloseTextDocument, listener),
    ...
    import_vscode16.languages.registerCodeActionsProvider(documentSelector, cargoTomlAction),
    // Register our versions completions provider
    import_vscode16.languages.registerCompletionItemProvider(
      documentSelector,
